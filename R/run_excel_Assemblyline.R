...
#'   An Excel metadata file provides metadata template files and data for the  
#'   EMLassemblyline(EAL) package.  The EML file and data files can be uploaded to
#'   the Environmental Data Initiative data portal. https://portal.edirepository.org/ 
#'   
#'   The Excel metadata file has a metadata sheet, a data sheet and look-up sheets
#'   for site locations and units.  Currently only one data sheet is supported.
#'   TO DO: add a link to documentation on using the metadata sheet.
#'   
#'    There are several choices for organizing data and template files.
#'    
#'    1)  The EAL suggested organization scheme is a single directory for each data package containing:
#'      * data_objects - A directory of data and other digital objects to be packaged 
#'        (e.g. data files, scripts, .zip files, etc.).
#'      * metadata_templates - A directory of EAL template files.
#'      * eml - A directory of EML files created by EAL. 
#'    2)  Customized directory structure, e.g. project sub-directories containing 
#'        directories of data packages.
#'    3)  A simpler organization scheme where a single directory contains all 
#'        files belonging to a data package.
#'    
#'    The structure can be customized by changing the directories (paths)  
#'
  ...
  # site_name is used to find the metadata sheet and should be in cell A1 of the sheet.
  ...
  EMLal_arguments <-  excel_meta_info$dset_info %>%
    ...
    list_modify(
      ...
      data.table.url = paste0(path_data,"/",.$data.table),
      data.table.quote.character = "\"",
      other.entity = c(wb_name),
      other.entity.name = c(str_replace(wb_name, "\\.", "_")),
      other.entity.description = c(paste(
        "Excel file with metadata and data sheets:", .$dataset.title
      )),
      other.entity.url = paste0(path_data,"/",.$other.entity),
      user.id = user_id,
  ...
  # -- Personnel.txt --------------
  #    Use information from excel file; looking up the NSF grant titles based on 
  #    award numbers.
  ...
  personnel <- excel_meta_info$dset_info %>% 
    rename(givenName=first_name,surName=last_name,organizationName=organization, 
                      electronicMailAddress=email, userId=orcid_id_optional) %>%
    select(givenName,surName,organizationName,electronicMailAddress,userId,role) %>%
    filter(!is.na(role))%>%
    mutate(middleInitial=NA, .after=givenName)%>%
    mutate(role = case_when(str_trim(role)=='Owner' ~ 'creator',
  ...
  project_funding <- excel_meta_info$dset_info %>% 
    select(pi_first_name,pi_last_name,
           title_of_grant,funding_agency,funding_number) %>%
    filter(!is.na(funding_number)) %>%
    rename( givenName = pi_first_name,
  ...
  if(nrow(project_funding)==0) warning("No project funding!! ")

  personnel.txt <- personnel %>%
    full_join(., as.data.frame(project_funding), copy = T) %>%
    select(
      givenName,middleInitial,surName,organizationName,electronicMailAddress,
      userId,role,projectTitle,fundingAgency,fundingNumber
    ...
    add_row(givenName="Information Manger",organizationName=site_name,
            electronicMailAddress=site_email, role="contact")
  
  write.table(personnel.txt,file = paste0(path_templates,"/personnel.txt"),sep = "\t ",
  ...
  geo_coverage <-  excel_meta_info$dset_info %>%
    rename(geographicDescription=geographic_description,
    ...
    filter(!is.na(geographicDescription),
           !geographicDescription=="Enter Description")%>%
    mutate(
    ...
    select(geographicDescription,northBoundingCoordinate,southBoundingCoordinate,
           eastBoundingCoordinate,westBoundingCoordinate)
  write.table(geo_coverage,paste0(path_templates,"/geographic_coverage.txt"),
  ...
  keywords<- excel_meta_info$dset_info %>%
    ...
    mutate(keywordThesaurus= case_when(
  ...
  write.table(keywords,paste0(path_templates,"/keywords.txt"),sep = "\t",
              row.names = F,quote = F)
  ...
  # Check for category variables. If there are code definitions it is
  # a categorical class
  ...
  write.table(select(excel_meta_info$attributes, -code_info),
              paste0(path_templates,"/attributes_",
                                tools::file_path_sans_ext(wb_name),".txt"),
              sep = "\t",row.names = F, na="",quote = F)
  write.table(cat_vars,
             paste0(path_templates,"/catvars_",
                    tools::file_path_sans_ext(wb_name),".txt"),
             sep = "\t",row.names = F, na="",quote = F)
  ...
  # Check for units not in ListUnitDictionary.csv
    undefined_units <- anti_join(custom_units.txt,unit_lookup,by="id")
  if (!nrow(undefined_units) == 0) {
    warning("Units not in unit lookup table.")
    undefined_units
    }
    
  write.table(custom_units.txt,
  ...
  # The publication date of the data set will be the date this script runs.
  # To use the publication date from excel file e.g, only minor corrections of 
  # metadata and data are not changed then run the following
